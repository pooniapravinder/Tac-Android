...
public class MutiMediaView extends FrameLayout {
    ...
    private TextView mTvtapVideoCrop;
    ...
    private void init() {
        ...
        mTvtapVideoCrop = findViewById(R.id.tap_video_crop);
        ...
        if (mRvSelectedView == null) {
            ...
            mSelectedVideoAdapter.setItemViewCallback(new SelectedMediaAdapter.OnItemViewCallback() {
                ...
                public void onDurationChange(long currDuration, boolean isReachedMaxDuration) {
                    if (currDuration > 0) {
                        mTvtapVideoCrop.setVisibility(VISIBLE);
                    } else {
                        mTvtapVideoCrop.setVisibility(GONE);
                    }
    ...
    public void addSelectMedia(MediaInfo info) {
        ...
        mRvSelectedView.post(new Runnable() {
            @Override
            public void run() {
                mRvSelectedView.smoothScrollToPosition(mRvSelectedView.getAdapter().getItemCount() - 1);
            }
        });
    }
